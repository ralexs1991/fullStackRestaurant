...
function MyApp(props){
  var   { cart,addItem,removeItem,removeItemAll, user, setUser } = useContext(AppContext)
  ...
  removeItemAll = (item) => {
      ...
      if (foundItem.quantity > 1) {
        var newCart = {
          items: items.map((item) =>{
          if(item.id === foundItem.id){
            return Object.assign({}, item, { quantity: item.quantity == 0 })
           }else{
          return item;
        }}),
        total: state.cart.total - item.price,
  ...
  return (
    // <AppContext.Provider value={{cart: state.cart, addItem: addItem, removeItem: removeItem,isAuthenticated:false,user:null,setUser:(user) => setState({ user })}}>
    <AppContext.Provider value={{cart: state.cart, addItem: addItem, removeItem: removeItem,removeItemAll, resetCart, isAuthenticated:false,user:stateUser,setUser:setUser}}>
      <Head>
        <link
          rel="stylesheet"
